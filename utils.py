...
def get_file_paths(directory: str, extension: str, keyword: str, print_paths=False, skip_mice: list = [], skip_sessions: list = []) -> list:
    all_paths = [f for f in Path(directory).glob(f'**/*.{extension}') if keyword in f.name]
    
    # Filter out files from mice or sessions that should be skipped
    filtered_paths = []
    for path in all_paths:
        path_parts = str(path).split(os.sep)
        
        # Extract mouse and session information from the path
        # Based on the example: "E:\clickbait-ephys\data\preprocessed\6000\11\mua.npy"
        # Mouse ID is typically at index -3 (e.g., "6000")
        # Session ID is typically at index -2 (e.g., "11")
        
        mouse_id = path_parts[-3] if len(path_parts) >= 3 else ""
        session_id = path_parts[-2] if len(path_parts) >= 2 else ""
        
        # Skip if mouse ID or session ID is in the skip lists
        if mouse_id in skip_mice or session_id in skip_sessions:
            continue
            
        filtered_paths.append(path)
    
    print(f'Found {len(all_paths)} {keyword}.{extension} files')
    print(f'After filtering: {len(filtered_paths)} files (skipped {len(all_paths) - len(filtered_paths)})')
    
    if print_paths:
        show_paths(filtered_paths)
